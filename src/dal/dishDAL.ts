...
class DishDAL {
    static async getDishById(dishId: string) {
        ...
        const model = await ModelModel.findOne({dishId}).select('-_id -dishId').lean() as Omit<IModel, 'dishId'>;
        const stories = await StoryModel.find({dishId}).select('-_id -dishId').lean() as Omit<IStory, 'dishId'>[];
        const videos = await VideoModel.find({dishId}).select('-_id -dishId').lean() as Omit<IVideo, 'dishId'>[];
        const quizzes = await QuizModel.find({dishId}).select('-_id -dishId').lean() as Omit<IQuiz, 'dishId'>[];
